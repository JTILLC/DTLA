...
export function buildMachineList(symptoms, entries, emailThreads) {
  const map = new Map();
  ...
  // Email threads arrive pre-tagged, so reuse the tokens rather than re-scanning
  // the bodies a second time.
  for (const t of emailThreads || []) {
    (t.machines || []).forEach(bump);
  }
  return [...map.entries()]
