...
export const screenInfo = {
  ...
  'run-weight': {
    ...
    keys: [
      ...
      { name: 'Live (app bar)', desc: 'With Live on, product moves through the weigher: the dispersion table is held at the DF target weight and throws product to the radial feeders in proportion to the DF time and amplitude; each feeder doses its pool hopper in proportion to its own time and amplitude, and more when product piles up; the pool hopper drops into the empty weigh hopper. The weights here are those hoppers. The rule of thumb is about three times the target on the DF - 300 g for this 90 g preset; the program\'s 500 g default floods the hoppers within a few dozen cycles. Raise the feeders or the DF target and the hoppers fill past a combination - missed cycles, then the Overweight error; a hopper past target + upper limit (93.3 g here) is the Overscale error and is dumped on clearing. Turn a feeder\'s time and amplitude down far enough and it moves no product at all, so its pool hopper stays empty and the cycles miss underweight.' },
    ],
  },
  ...
  'display-preset': {
    ...
    keys: [
      ...
      { name: 'All Select', desc: 'Turns every Source row blue; Copy then writes all ten slots onto the Destination side, slot for slot. (The key did nothing visible on the running program.)' },
      { name: 'Initialize', desc: 'Asks "Initialize the Memory. OK?" Yes wipes every Memory slot.' },
      { name: 'Leaving the tab', desc: 'Going to another tab or screen puts both stores and the Memory / Card choices back to their defaults, so every trainee starts from the same place.' },
    ],
    note: OBSERVED_NOTE,
  },
  ...
  'display-machine-edit': {
    ...
    keys: [
      ...
      { name: 'Copying (by analogy)', desc: 'On the running program this tab is inert - nothing on it responds - so it behaves here as the Preset Manager does: Memory holds the ten setting groups, the Card starts empty, a tap on a row on each side picks the copy source and destination, All Select takes every Source row, Copy asks first, and an empty source copied over a slot empties it. Leaving the tab puts both stores back to their defaults.' },
    ],
    note:
