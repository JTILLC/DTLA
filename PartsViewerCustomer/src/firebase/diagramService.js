...
/**
 * Load diagrams for a specific customer from Firebase
 * @param {string} customerName - The customer name to filter by
 * @returns {Promise<Array>}
 */
export const loadDiagramsByCustomer = async (customerName) => {
  try {
    console.log(`[Firebase] Loading diagrams for customer: ${customerName}`);
    const q = query(
    ...
    console.log(`[Firebase] Loaded ${diagrams.length} diagram(s) for customer: ${customerName}`);
    return diagrams;
  } catch (error) {
    console.error('Error loading diagrams by customer:', error);
    throw error;
  }
};

/**
 * Get list of all unique customer names from Firebase
 * @returns {Promise<Array<string>>}
 */
export const getCustomerNames = async () => {
  try {
    console.log('[Firebase] Loading customer names...');
    const querySnapshot = await getDocs(collection(db, DIAGRAMS_COLLECTION));

    const customers = new Set();
    querySnapshot.forEach((doc) => {
      const customer = doc.data().customer || 'General';
      customers.add(customer);
    });

    const customerList = Array.from(customers).sort();
    console.log(`[Firebase] Found ${customerList.length} customer(s):`, customerList);
    return customerList;
  } catch (error) {
