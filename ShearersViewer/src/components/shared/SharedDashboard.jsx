...
export default function SharedDashboard({ data = {}, dates = [], selectedDate: parentSelectedDate = '' }) {
  // Use parent's selectedDate if provided (single date), otherwise use internal state
  const [internalSelectedDate, setInternalSelectedDate] = useState(dates.length ? dates[0] : '');
  const [openSections, setOpenSections] = useState([]);

  // If parent passes a single date, use that; otherwise use internal state with "All Days" option
  const showDateSelector = dates.length > 1 || parentSelectedDate === 'All Dates';
  const selectedDate = dates.length === 1 ? dates[0] : internalSelectedDate;

  // Update internal selected date when dates change
  React.useEffect(() => {
    if (dates.length > 0 && !dates.includes(internalSelectedDate)) {
      setInternalSelectedDate(dates[0]);
    }
  }, [dates]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
      ...
      {/* Date selector - only show if multiple dates */}
      {showDateSelector && (
        <div className="flex justify-center items-center mb-4 space-x-4">
          <label className="font-medium dark:text-gray-200">View:</label>
          <select
          ...
          >
            {dates.length > 1 ? <option value="All Days">All Days</option> : null}
