...
export default function SharedRunning({ data = {}, dates = [], selectedDate: parentSelectedDate = '' }) {
  const [internalSelectedDate, setInternalSelectedDate] = useState(dates[0] || '');
  const [expandedSections, setExpandedSections] = useState([]);

  // Use the first (or only) date from filtered dates
  const selectedDate = dates.length === 1 ? dates[0] : internalSelectedDate;
  const showDateSelector = dates.length > 1;
