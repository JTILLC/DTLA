...
import { Save, CloudUpload, CloudDownload, Copy, RefreshCw, Trash2, Edit3, Plus, Download, Upload, FileText, History, Settings, Eye, HelpCircle } from 'lucide-react';
...
const addLine = (setLines, setActiveLineId, globalData, lines) => {
  const lineName = window.prompt('Enter line name:', `Line ${lines.length + 1}`);
  if (!lineName || lineName.trim() === '') return; // Cancel if empty

  const headCountInput = window.prompt('How many heads?', globalData.headCount || '14');
  if (!headCountInput) return; // Cancel if empty

  const headCount = parseInt(headCountInput);
  if (isNaN(headCount) || headCount < 1) {
    alert('Please enter a valid number of heads (must be at least 1)');
    return;
  }
  ...
  const newLine = {
    ...
    running: false,
    notes: '',
    heads: Array.from({ length: headCount }, (_, i) => ({
      id: i + 1,
      status: 'active',
      error: 'None',
      notes: '',
...
const exportLineToPDF = (line, globalData) => {
  ...
  // Line details
  doc.setFontSize(10);
  doc.text(`Model: ${line.model || 'N/A'}`, 14, y);
  y += 6;
  doc.text(`Job Number: ${line.jobNumber || 'N/A'}`, 14, y);
  y += 6;
  doc.text(`Serial Number: ${line.serialNumber || 'N/A'}`, 14, y);
  y += 6;
  doc.text(`Running: ${line.running ? 'Yes' : 'No'}`, 14, y);
  y += 10;
  ...
  // Issue heads
  const issueHeads = line.heads.filter(head =>
    head.status !== 'active' || head.error !== 'None' || head.notes.trim() !== '' || head.fixed !== 'na'
  );
...
const App = () => {
  ...
  return (
    <div className="container-fluid p-0">
      ...
      <Tabs defaultActiveKey="current" className="mb-1 border-bottom">
