...
export default function SharedViewer() {
  ...
  useEffect(() => {
    ...
    get(ref(database, `jti-downtime/shares/${token}`)).then((snapshot) => {
      ...
      // If the component unmounted while awaiting the token check, detach now.
      if (cancelled) {
        unsubscribe();
        unsubscribe = null;
      }
    ...
    // Cleanup from the effect itself (the old code returned this from inside
    // the .then(), so it never ran and the listener leaked on every unmount).
    return () => {
      cancelled = true;
