  ...
  const runningLines = useMemo(() => {
    return lines
      .map((line) => {
        ...
        const fixed = offline.filter((h) => (h.repaired ?? 'Not Fixed') === 'Fixed');
        return {
          line,
          heads,
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-md md:p-4 sm:p-2">
      <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
        <h2 className="text-2xl font-semibold sm:text-xl">Running Heads</h2>
        <div className="flex items-center gap-2">
          <label className="font-medium text-sm">Date:</label>
          <select
            value={selectedDate}
            onChange={(e) => setSelectedDate(e.target.value)}
            className="border p-2 rounded sm:p-1 sm:text-sm"
          >
            {(dates || []).map((d) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                <div className="flex flex-wrap gap-2 text-sm">
                  <span className="bg-red-100 text-red-700 px-2 py-1 rounded">Offline: <b>{offlineCount}</b></span>
                  <span className="bg-orange-100 text-orange-700 px-2 py-1 rounded">Fixed: <b>{fixedCount}</b></span>
                  <span className="bg-yellow-100 text-yellow-700 px-2 py-1 rounded">Not Fixed: <b>{notFixedCount}</b></span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-7 gap-2">
                {heads.slice(0, HEADS_PER_LINE).map((h) => (
                  <div
                    ...
                    title={`Head ${h.head} • ${h.offline === 'Active' ? 'Active' : (h.repaired === 'Fixed' ? 'Fixed' : 'Offline')} • ${h.issue || 'None'}`}
                  >
                    {h.head}
                  </div>
                ))}
