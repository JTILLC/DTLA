      ...
      Object.keys(dayData).forEach((line) => {
            ...
            if (issuesArray.length === 0) {
              ...
              issuesArray.forEach((iss, idx) => {
                ...
                result.push({
                  ...
                  running: 'Yes',
                  photos: (iss.photos || []).filter((ph) => ph && ph.url),
                });
              });
            }
          });
        ...
        if (running && entry.machineNotes && entry.machineNotes.trim()) {
          result.push({
            ...
            repaired: '',
            running: 'Yes',
            photos: (entry.notePhotos || []).filter((ph) => ph && ph.url),
          });
        }
      });
    });

    // Newest -> Oldest, matching the main Summary page.
    result.sort((a, b) => {
          ...
          <table className="w-full table-auto border-collapse min-w-max">
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-700">
                {['Date', 'Line', 'Head', 'Issue', 'Notes', 'Repaired', 'Running', 'Photos'].map((c) => (
                ...
                return (
                  <tr key={r.key} className={rowClass}>
                    ...
                    <td className="p-2 border dark:border-gray-600">
                      {r.photos && r.photos.length ? <PhotoThumbs photos={r.photos} /> : <span className="text-gray-500">—</span>}
                    </td>
                  </tr>
                );
              })}
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  {r.photos && r.photos.length > 0 && (
                    <div className="col-span-2">
                      ...
                      <div className="mt-1"><PhotoThumbs photos={r.photos} /></div>
                    </div>
                  )}
                </div>
              </div>
