...
function InvoicePage() {
  ...
  const generatePDF = (preview = false) => {
    try {
      ...
      doc.setFontSize(9);
      doc.text(`${COMPANY_INFO.name} Invoice`, 10, yOffset);
      yOffset += 4;
      doc.text(`Address: ${COMPANY_INFO.address} ${COMPANY_INFO.cityStateZip}`, 10, yOffset);
      yOffset += 4;
      doc.setFontSize(8);
      ...
      yOffset += 4;
      doc.text(`Email: ${COMPANY_INFO.email}`, 10, yOffset);
      doc.text(`Service Dates: ${invoiceInfo?.serviceDates || 'N/A'}`, 100, yOffset);
      yOffset += 4;
      doc.text(`Phone: ${COMPANY_INFO.phone}`, 10, yOffset);
      ...
      autoTable(doc, {
        ...
        body: [
          ['Straight', charges.straight?.hours.toFixed(2) || '0.00', `$${LABOR_RATES.STRAIGHT}`, `$${charges.straight?.charge.toFixed(2) || '0.00'}`],
          ['Sat/OT', charges.overtime?.hours.toFixed(2) || '0.00', `$${LABOR_RATES.OVERTIME}`, `$${charges.overtime?.charge.toFixed(2) || '0.00'}`],
          ['Sun/Hol', charges.double?.hours.toFixed(2) || '0.00', `$${LABOR_RATES.DOUBLE}`, `$${charges.double?.charge.toFixed(2) || '0.00'}`],
      ...
      autoTable(doc, {
        ...
        head: [['Category', 'Hours', 'Rate', 'Charge']],
        body: [
          ['Weekday', charges.weekdayTravel?.hours.toFixed(2) || '0.00', `$${TRAVEL_RATES.WEEKDAY}`, `$${charges.weekdayTravel?.charge.toFixed(2) || '0.00'}`],
          ['Saturday', charges.saturdayTravel?.hours.toFixed(2) || '0.00', `$${TRAVEL_RATES.SATURDAY}`, `$${charges.saturdayTravel?.charge.toFixed(2) || '0.00'}`],
          ['Sun/Hol', charges.sundayTravel?.hours.toFixed(2) || '0.00', `$${TRAVEL_RATES.SUNDAY_HOLIDAY}`, `$${charges.sundayTravel?.charge.toFixed(2) || '0.00'}`],
      ...
      autoTable(doc, {
        ...
        body: [
          [
            ...
            `${travelData?.perDiemType === 'local' ? `$${PER_DIEM_RATES.LOCAL}/day` : `$${PER_DIEM_RATES.NON_LOCAL}/day`}, ${travelData?.perDiemDays || 0} days`,
          ],
          ['Mileage', `$${charges.travel?.mileageTotal.toFixed(2) || '0.00'}`, `${travelData?.mileage || 0} miles at $${(charges.travel?.mileageRate ?? DEFAULTS.MILEAGE_RATE).toFixed(2)}`],
      ...
      // Footer Text
      doc.setFontSize(8);
      doc.text(`If not paying by ACH make checks payable to ${COMPANY_INFO.name}`, 10, yOffset);
      yOffset += 3;
      doc.text(`Any questions or concerns please contact Josh Lemmons at ${COMPANY_INFO.phone}`, 10, yOffset);
      yOffset += 3;
      doc.text(`EMAIL: ${COMPANY_INFO.email}`, 10, yOffset);
      yOffset += 3;
